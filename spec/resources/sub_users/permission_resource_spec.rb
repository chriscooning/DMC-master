...
# sub users 
describe SubUsers::PermissionResource do
  ...
  context "index" do
    it "available for account owner" do
      expect {
        SubUsers::PermissionResource.new(as: owner, account: account).read_all
      }.not_to raise_error
    end
    ...
    it "available for user with 'view_analytics' permission" do
      ...
      expect {
        SubUsers::PermissionResource.new(as: user, account: account).read_all
      }.not_to raise_error
    end
  ...
  context "edit" do
    it "available for account owner" do
      expect {
        SubUsers::PermissionResource.new(as: owner, account: account).build
      }.not_to raise_error
    end
    ...
    it "available for user with 'view_analytics' permission" do
      ...
      expect {
        SubUsers::PermissionResource.new(as: user, account: account).build
      }.not_to raise_error
    end
